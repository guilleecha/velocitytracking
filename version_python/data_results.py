...
#        Angles = data_filtered["timestack" + str(t)]["Peaks"][indices,0]
        ...
        if velocity_peaks[indices].any():
            ...
            bins_range = 0.05 #separacion entre bins
            bins_number = int((hist_range[1]-hist_range[0])/bins_range)
            hist, bins = np.histogram (velocity_peaks[indices],density = True,bins = bins_number , range = hist_range )
